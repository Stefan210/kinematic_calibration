...
void CameraTransformOptimization::printSummarziedResultCSV(std::string pre,
		const CalibrationState& state, tf::Vector3 markerPosition) {
...
void CameraTransformOptimization::printResultCSV(std::string pre,
	...
	if (!headlinePrinted) {
		std::cout << "file,description,marker_x,marker_y,marker_z,"
				"headyaw_offset,headpitch_offset,"
				"tx,ty,tz,rr,rp,ry,markerError,"
				"groundAngleError,groundDistanceError\n";
		headlinePrinted = true;
	...
	int numOfPoints = measurePoints.size();
	for (int i = 0; i < numOfPoints; i++) {
		MeasurePoint& measurePoint = this->measurePoints[i];
		tf::Transform opticalToFixed = measurePoint.opticalToFixed(state);

		// Calculate distance from estimated marker.
		tf::Vector3 transformedPoint = opticalToFixed
				* measurePoint.measuredPosition;
		markerError = 0;
		markerError += pow(markerEstimate.x() - transformedPoint.x(), 2);
		markerError += pow(markerEstimate.y() - transformedPoint.y(), 2);
		markerError += pow(markerEstimate.z() - transformedPoint.z(), 2);
		markerError = sqrt(markerError);
		...
		std::cout << markerEstimate[0] << "," << markerEstimate[1] << ","
				<< markerEstimate[2] << ",";
		std::cout << state.getHeadYawOffset() << ","
				<< state.getHeadPitchOffset() << ",";
		std::cout << cameraToHead.getOrigin()[0] << ","
