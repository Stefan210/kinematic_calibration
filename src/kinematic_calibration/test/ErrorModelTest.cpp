...
TEST(ErrorModel, getImageCoordinatesTest) {
	// arrange
	...
	for(map<string, double>::iterator it = pos.begin(); it != pos.end(); it++) {
		measurement.jointState.name.push_back(it->first);
		measurement.jointState.position.push_back(it->second);
	}

	KinematicChain chain(tree, measurement.chain_root,
			measurement.chain_tip, measurement.chain_name);
