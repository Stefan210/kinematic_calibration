...
CSVRESULT='result_test_data.csv'
rm $CSVRESULT
...
do
      ...
      AVGRMS=`awk '{mean += $18} END {print mean/(NR-1);}' $CSVOUT`
      VAR=`awk -v CONVFMT=%.28g '{sum+=$18; sumsq+=($18*$18);} END {print (sumsq/(NR-1) - (sum/(NR-1)*sum/(NR-1)));}' $CSVOUT`
